...
export function AllyGrid({ variant = "home" }: Props) {
  ...
  let resto = allAliados.filter(
    (a) =>
      !SPONSORS_IMPULSAN.includes(a.slug) &&
      !EXCLUIR_HOME.includes(a.slug) &&
      !a.categoriaAuditiva
  );
  ...
  return (
    <>
      ...
      {sponsors.length > 0 && (
        <Section background="default" ariaLabelledBy="ally-impulsan-title" className="py-14 md:py-16">
          <div className="max-w-5xl mx-auto">
            <h2 id="ally-impulsan-title" className="font-display text-3xl md:text-4xl font-extrabold text-ink text-center mb-10">
      ...
      {/* 2. Red de Empresas que Escuchan (las que apoyan mes a mes) */}
      <AliadosAuditivos />

      {/* 3. Empresas que nos acompañaron */}
      {resto.length > 0 && (
        <Section background="default" ariaLabelledBy="ally-acompanaron-title" className="py-14 md:py-16">
          <div className="max-w-5xl mx-auto">
            <h2 id="ally-acompanaron-title" className="text-sm font-display font-bold text-center mb-6 text-ink-soft uppercase tracking-[0.2em]">
